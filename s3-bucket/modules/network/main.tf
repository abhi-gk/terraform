...
resource "aws_subnet" "public_subnet" {
  ...
  tags = {
    Name = "${var.tagName}-PUBLIC-SUBNET-${count.index + 1}",
    ENV = "${var.EnvName}"
  }
...
/*    tags = {
        Name =  "OMPRKASHPROJECT-PUBLIC-ROUTE-TABLE"
    }*/
}
...
resource "aws_subnet" "private_subnet" {
  ...
  availability_zone = data.aws_availability_zones.all.names[count.index]
  tags = {
    Name = "${var.tagName}-PRIVATE-SUBNET${count.index + 1}",
    ENV = "${var.EnvName}"
  }
...
/*    tags = {
        Name = "OMPRAKASHPROJECT-PRIVATE-ROUTE-TABLE"
    }*/
}
...
resource "aws_nat_gateway" "public" {
  ...
  depends_on = [aws_internet_gateway.my_igw]
   ...
   tags = {
    Name = "${var.tagName}-PUBLIC-NAT-GATEWAY",
    ENV  = var.EnvName
   }
